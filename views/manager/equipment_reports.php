  ...
  <div style="max-width:1400px;margin:0 auto">
    ...
    <div style="background:#fff;padding:32px;border-radius:16px;box-shadow:0 4px 6px rgba(0,0,0,0.07);margin-bottom:32px">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px">
        <div>
          <h1 style="color:#2d3b7a;font-size:32px;margin:0 0 8px;font-weight:700">📊 Equipment Reports</h1>
          <p style="color:#6b7280;margin:0;font-size:15px">Track equipment condition logs and booking history</p>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap">
          ...
          <a href="?page=manager_add_equipment_report" style="display:inline-flex;align-items:center;gap:8px;background:#2d3b7a;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:600;box-shadow:0 4px 0 rgba(0,0,0,0.1)">
            <span style="font-size:20px">➕</span> Add Report
          </a>
        </div>
      </div>
    </div>
    ...
    <div style="background:#fff;border-radius:16px;box-shadow:0 4px 6px rgba(0,0,0,0.07);overflow:hidden">
      ...
      <?php if (empty($reports)): ?>
        <div style="padding:60px 20px;text-align:center">
          ...
          <p style="color:#9ca3af;margin:0 0 24px">Start by adding your first equipment report.</p>
          <a href="?page=manager_add_equipment_report" style="display:inline-block;background:#2d3b7a;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:600">
            ➕ Add First Report
          </a>
        </div>
      <?php else: ?>
                ...
                <tr style="border-bottom:1px solid #e5e7eb;transition:background 0.2s" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background='#fff'">
                  ...
                  <td style="padding:20px;text-align:center">
                    <div style="display:inline-flex;gap:8px">
                      <a href="?page=manager_view_equipment_report&id=<?php echo $report['id']; ?>" style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;background:#dbeafe;color:#1e40af;border-radius:8px;text-decoration:none;font-size:16px;transition:all 0.2s" title="View Details">👁️</a>
                      <a href="?page=manager_edit_equipment_report&id=<?php echo $report['id']; ?>" style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;background:#fef3c7;color:#92400e;border-radius:8px;text-decoration:none;font-size:16px;transition:all 0.2s" title="Edit">✏️</a>
                      <a href="?page=manager_delete_equipment_report&id=<?php echo $report['id']; ?>" onclick="return confirm('Are you sure you want to delete this report?')" style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;background:#fee2e2;color:#991b1b;border-radius:8px;text-decoration:none;font-size:16px;transition:all 0.2s" title="Delete">🗑️</a>
                    </div>
                  </td>
                </tr>
              <?php endforeach; ?>
